...
posts_asset = Asset('s3://databricks-airflow-project/raw/Posts.xml')
users_asset = Asset('s3://databricks-airflow-project/raw/Users.xml')
...
def produce_data_assets():
    ...
    s3_hook.load_file(
        filename = posts_file,
        key = 'raw/Posts.xml',
        bucket_name = 'databricks-airflow-project',
        replace = True 
    )
    s3_hook.load_file(
        filename = users_file,
